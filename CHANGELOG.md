...
- `aid advise` and `aid run` resolve the model through one function; advise reports the model that will actually run, whether it is pinned, and its source (explicit, sticky, budget route, CLI config, or agent default)
- Served-but-uncatalogued models are listed as unrated, and an agent CLI's configured default (e.g. codex `config.toml`) is reported as the default model
- Route evidence is reported only when observed: quota without a probe reads `unknown`, a not-signed-in failure is recorded as `auth: failed` for one hour, not-installed agents are ineligible, and `claude` appears in `aid agent list`
- Advise excludes weaker models on the caller's own provider pool when the caller model is known (`--caller-model` / `AID_CALLER_MODEL`) and demotes them otherwise
- Prices come only from exact matches or explicit overrides; unknown prices are stored as unknown, and a `max_task_cost` that cannot be enforced for an unpriced route is reported at dispatch
- Cursor availability uses one binary predicate across advise, agent list and preflight; the client shows an unknown quota as neutral
...
- AWAIT false positives on buffered agents are gone (wi-e49c): agy and grok print one line to the PTY and then work in their log, and the idle prompt detector marked that line as a prompt after 2 s, so tasks showed AWAIT for their whole run. The idle prompt now requires the agent's logs to have been silent for the task's idle-warn window (180 s by default; a real agy log measured gaps up to 145 s while working), and a task already in AWAIT returns to RUNNING when its logs grow. Immediate prompt shapes (`Continue? `, `(y/n)`) and streaming agents are unchanged.
- `aid run --dir <other repo>` no longer injects the caller's cwd project (wi-106c): rules, knowledge, project state and default skills are resolved once per dispatch from the `--dir` checkout — the same resolution verify, team and timeouts already used — and from no project at all when `--dir` is outside a git repository.
- A logged-out oz is held instead of dispatched (wi-69c7): `Your credentials are invalid` is a NeedsHuman signature, and every NeedsHuman hold now reads `needs human: <reason> — fix, then aid config clear-limit <agent>` on `aid agent list`, the session-start hook line and `aid doctor` instead of a bare `LIMITED`. `--urgency background` still keeps a clock or windowed hold selected but no longer executes an agent whose hold only a person can lift.
- `aid gc --task` succeeds after a sibling task already removed the shared worktree (wi-4df0): a worktree absent on disk and unregistered is treated as already collected — the commit, its ref and the manifest are still proven against the repository's common git dir and durability is recorded; only the clean-tree and live-HEAD checks are skipped. gc never runs a repository-wide `git worktree prune`, and a failing `git worktree list` is an error, not absence.
- `scripts/release.sh --dry-run` fails on orphan hygiene exactly like the real release (wi-5eef): it prints `dry-run: would fail: release hygiene check (N orphan branches, M orphan worktrees)` and exits non-zero, maps each orphan worktree to its task ids with the `aid accept` / `aid gc --task` commands, compares paths canonically, and treats a worktree whose `.aid-lock` names a live pid as held by a running task rather than an orphan.


## v10.46.1 (2026-09-14)
- Project config reaches every checkout: when a linked worktree (including a `git worktree add --detach` checkout) has no `.aid/project.toml` of its own — typically because the repo excludes `.aid/` from git — aid now loads the main working tree's `.aid/project.toml`. A local copy still wins. This closes the hole where `aid run --dir <detached checkout>`, the container project id and `[backup]` all ran with no project defaults. The main-working-tree resolver is shared between config discovery and project identity (`src/project/worktree.rs`); `resolve_project_id` stays main-first.
...
- Derive the dispatch model-info line from a new `Agent::default_model()` instead of a hard-coded table; self-heal retries are labelled as such and custom/delegate agents with an unknown default no longer print "CLI default".
- `scripts/remote-test.sh` and the project verify (`.aid/project.toml`) run the suite on the build box through `rbox exec` as a non-root build user with a real git checkout; the six root/no-`.git` test skips are gone.
- Create and probe every directory aid grants to a sandboxed agent (cargo target `_base`/branch leaf, a task-private TMPDIR) before launch, and pass codex writable roots for regular checkouts too; a granted-but-absent directory used to be unwritable and silently forced a cold temp build.


## v10.43.0 (2026-09-11)
- Fix `aid run` / `aid batch` silently pinning a stale catalog model for a declared `standard` or `premium` budget (codex standard → gpt-5.6-luna, premium → gpt-5.6-sol) so the CLI's own default (e.g. codex `gpt-6-astra`) was unreachable without `-m` or an agent_config default. Standard/premium now leave the model unset; free/cheap still use the catalog; `-m` > agent_config precedence is unchanged. `healthy_model_for` treats an unset model as the agent's default quota group and no longer emits a spurious "model group exhausted" switch when that group is healthy; the held-route fallback pin is preserved. Every explicit-agent dispatch prints the effective model and its source. Simple-task smart routing applies only when no budget is declared.
- Fix `build.rs` embedding the entire `CHANGELOG.md` into one rustc environment variable when git tags were unavailable (source archives, crates.io tarballs): the 218 KB value exceeded Linux's 128 KiB per-string limit and every build of the crate failed with `Argument list too long`. The embedded changelog is now always the newest ten `CHANGELOG.md` releases, written to `OUT_DIR` and included with `include_str!`, so in-repo builds and source archives embed the same bounded text; `aid changelog --git` still reads live tags.
- Release tooling: `scripts/release.sh` honours `AID_RELEASE_TEST_CMD`, and the new `scripts/remote-test.sh` ships the tracked tree over Tailscale SSH and runs `cargo test --workspace` on a build box, so the release suite no longer has to compile on the releasing machine.
...
- Load job specs written by older builds. A removed spec field must not make an in-flight upgrade fail: a single unreadable spec previously aborted `aid board`, `aid stop`, `aid unstick` and every reaper pass.
- Delete the foreground detach machinery it replaces: `handle_foreground_detach`, `should_detach`, `adopt_detached_task` and the `detached` spec marker.
- Carry `budget`, `session_id`, `max_task_cost`, `audit_explicit` and `no_audit` into background specs, so foreground and background runs no longer diverge; `--timeout` keeps its exact seconds instead of truncating to whole minutes.
- Add `tests/foreground_worker_survival_e2e.rs`: real signal-level acceptance for tree-kill survival, SIGINT stop, and the foreground exit-code and stdout contract, with teardown that leaves no orphaned processes.


## v10.39.0 (2026-09-03)
- Fix the isolated-`$HOME` symlink leak that made an agent CLI undispatchable host-wide. The isolated home is a symlink farm over the operator's real home, so an agent that self-installs writes its payload into the real `~/.local` but records the task-home path as the link target; removing the task home then leaves a dangling entry in the operator's `PATH` forever. Observed twice for real, four weeks apart, both cursor. Every path that removes a task home — `IsolatedHomeGuard::drop`, the stale-home pre-clear, and `aid clean` — now rewrites those links back to the real home first, matching by path component rather than string prefix.
- Only rewrite a link when the rewritten target exists and is not the link itself; anything else is left byte-identical, warned about, and excluded from the repaired count. An unrecognised path shape is refused rather than guessed at, and `..` / `.` components and empty remainders are rejected outright.
- Never remove an isolated home after a sweep that did not complete. A failed sweep followed by the delete is how the original dangling link comes back, so the home is now preserved and `aid clean` retries it later.
- `aid doctor` reports symlinks in `~/.local/bin`, `~/bin` and `~/.cargo/bin` that point into any `~/.aid/tasks/*/home` or `~/.aid/tmp_home/*` path, with the rewrite that would repair each; `aid doctor --apply` performs it. This is the only thing that can repair damage left by a task home that is already gone.
- Resolve Cursor's `agent` binary by walking `$PATH` and identity-probing each candidate instead of trusting the first hit. Cursor's installer no longer creates `cursor-agent`, and xAI's Grok CLI installs a binary called exactly `agent`; when it came first in `PATH`, aid reported cursor uninstalled and refused every dispatch while a working install sat on disk.


## v10.38.0 (2026-09-02)
- qwen dispatches can write files again. qwen-code 0.22.3 defaults a non-interactive `-p` run to `permission_mode: auto`, which ships no `write_file`, no `edit` and no shell tool, and aid never passed an approval-mode flag — so the model read the repo, wrote a correct plan, explained it could not write, and exited 0. Every one of the 27 qwen tasks dispatched between 2026-08-07 and 2026-09-02 changed zero files and every one was recorded `done`. Normal dispatches now pass `-y`; `--read-only`, which the adapter used to refuse outright, maps to qwen's native `--approval-mode plan`, and the result-file exception is preserved as it is for gemini and agy. `-y` is absent from `qwen --help` and still accepted by its parser — verified against the live CLI, not read from the docs.
- A qwen run whose token plan is exhausted is recorded as failed instead of done. The quota channel opens one slot for qwen's terminal envelope and read it as `envelope["text"]`; qwen emits that text under `result`, so the slot had never returned anything and no `rate-limit-qwen` marker had ever been written. Ten exhausted runs were stored `done` with exit 0. The tests that covered the slot asserted against a `{"type":"result","text":…}` shape qwen does not emit; they now run off a real captured envelope, and a third test that bypassed the channel splitter entirely was rerouted through it.
- `aid show --output` no longer reports "unrecognized JSON log format" for an agent aid already identified, and no longer prints a claude report twice. The output extractor now reads a terminal envelope's top-level `result`, and the recorded agent name is passed down instead of `None` — aid had been sniffing for an agent whose kind was on the task row. The terminal text is emitted only when no assistant content was collected: claude and qwen carry the same text in both places, so an unconditional read doubled the saved `output.md` for every claude task, and commandcode's `finalText` had to keep flowing unconditionally.
- A task whose worker died no longer reads RUN for a day. The orphan reaper skipped any task with no background spec, and the spec is cleared by a Drop guard when the worker exits — so a worker that died after clearing it left the row unreachable until the 24-hour hard cap. Measured on this machine: four running rows, one silent for 14 hours. Spec-less tasks are now judged by the same liveness signals as the rest, and yield to the hard-cap path past 24 hours so that "idle past its window" and "exceeded maximum runtime" stay two different recorded facts. A live foreground run is unaffected: its spec exists, so its own `--idle-timeout` still governs.
- An agy child that reports a terminal executor failure in its private diagnostic log and exits 0 is recorded as failed. With partial stdout and a zero exit code the watcher had recorded such a task Done with verify passed, so a run that never did the work was indistinguishable from one that did. Agents now declare whether their diagnostic log proves terminal failure; agy recognises only the exact `agent executor error:` marker, and an ordinary recovered tool error stays a success. The three fixtures are real captured children.
- An explicit `--dir` into another repository now uses that project's own configuration. Cross-repository dispatch inherited the caller's verify command, agent cache, worktree seed and managed Cargo target namespace, so work sent into another repo was verified with the wrong command and built into the wrong cache. `target_dir_for_worktree` is replaced by `target_dir_for_project` at every call site, and there is no caller-config fallback when the explicit target has no config of its own.
- AID Command ships: a macOS and iPadOS console over a new authenticated client HTTP API, with live fleet state over SSE, Keychain-backed auth, and Hangar / Cargo / Mission Brief screens. The live acceptance probe now owns exactly one server process, waits for authenticated readiness and reaps that pid rather than matching broadly, so a probe phase can no longer report against a server it did not start.


## v10.37.0 (2026-08-19)
- A foreground `aid run` or `aid retry` killed by the caller's timeout no longer takes the agent with it. The spec records a deliberate detach, aid exits without signalling the agent, and the reaper adopts the task instead of reaping it. Before this, the agent died and the task recorded STOP — and the reattach hint aid printed, `aid watch --wait <id>`, was itself the next `aid` process whose zombie sweep did the killing.
- An adopted task whose agent exited unobserved no longer reads as success. A kill, a crash and a clean finish are indistinguishable once the worker that would have captured the exit code is gone, so unless a Completion event survived the detach the task records `Unverified(NoResult)` with verify status `unobserved`: `aid board` shows `DONE [VNORESULT]` and does not count it done, `aid watch` exits non-zero, and the JSON surfaces carry `"outcome":"unverified"`.
- An orphan agent with no detach marker is still killed and its task still recorded as failed. The existing regression test passed a pid that was never alive, so nothing pinned the live case; widening the adoption gate now turns the new test red.
- droid's weekly Core 402 holds only the Core pool instead of parking Standard with it. Captured live: `You've reached your weekly Droid Core usage limit (resets in 2 days)`. The standard refusal names Droid Core inside its own body, so the needles are ordered to keep a standard refusal out of the Core pool, and a droid refusal that names neither tier still holds the whole agent.


## v10.36.0 (2026-08-19)
- droid is no longer written off for a week by a single refusal. Factory meters a `standard` pool and a `core` pool separately, so a weekly or 5-hour standard 402 now holds only the standard tier: `aid agent list` reports droid `PARTIAL`, and dispatch stays on droid and switches to a Core model instead of leaving for another agent. Measured before the fix: droid took zero dispatches for three days under an agent-wide hold while its Core pool sat at 1% used and served free.
- Core membership comes from Factory's own `billingPool` table and its `?? "standard"` default, not from probing. A model Factory adds upstream is therefore classified as standard until Factory says otherwise, and an unknown, aliased, whitespace-padded, or router (`auto`) name never lands in the live tier.
- droid's model catalog grew from 4 rows — three of them CLI aliases — to the 44 models droid 0.199.0 actually serves. No per-token prices are invented: Factory publishes none and `estimate_cost` already returns `None` for droid, so the rows carry "no figure" the way the grok rows do. `--budget` now selects a Core model, the one tier that survives a spent standard pool.
- A droid refusal that names no tier — `402 payment required: reload your tokens`, which a purchase ends rather than a clock — holds the whole agent again even when a model was on the dispatched route. Keying holds by metered unit had made the dispatched model always win, which would have kept sending Core work at an account that can serve nothing.
- `aid agent list` and `aid advise` now read droid's real quota from aidbar's `droid` probe, which reports Factory's standard and core windows with a published window end, instead of printing `no probe` and recovering the hold on a cooldown guessed from the refusal text. Headroom on the core pool still does not release a standard hold.


## v10.35.0 (2026-08-18)
- `aid agent list`, `aid advise`, and cost reporting now see the models the opencode CLI actually serves: the catalog grew from five `opencode/*` rows to 187, including the `opencode-go/*` provider that every dispatch on a Google-One-style setup depends on. A discovered model aid cannot price reads as unknown rather than free, a static catalog row's own price outranks discovery, and a self-declared free name is recognised by suffix (`-free`, `/free`, `:free`) for every provider instead of an arbitrary family allowlist.
- A held-route substitution no longer loses its model to a served-list miss. The pin aid chooses when it re-routes away from an exhausted model family is kept when validation cannot confirm it, because a missing served-list entry is cache lag rather than proof the model is unservable — and dropping it sent the task back to the agent's own default, which is the family the substitution had just escaped.
- A quota hold no longer promises a recovery aid cannot observe. A refusal classified as windowed is released only by a dated usage snapshot with headroom, so on a route with no live quota probe that condition can never be met; such a hold now states plainly that it needs `aid config clear-limit`, and `aid agent quota` says so too.
...
- OpenCode prepaid, Copilot monthly, and Gemini `IneligibleTier` stay person-only. A bare 429 is no longer a hold and no longer scores −10.
- `aid advise` ranks remaining headroom without boosting unused quota, and each candidate JSON object now carries a `quota` field.
- `aid agent quota` prints live used percent and probe age. Background wait no longer loops on a prepaid hold. Dispatch never probes the provider.
- Adapters mark quota only from provider refusal channels, so model-authored text cannot forge a hold.
