...
- Clean up before releasing. The orphan hygiene check **fails both `--dry-run` and the real release** if a branch already merged into `main` still exists or an orphan worktree remains; `--skip-hygiene` is the only bypass. A worktree whose `.aid-lock` names a live pid (and that branch) is held by a running task, not an orphan. Delete merged branches, `aid accept` then `aid gc --task <id>` any task still holding worktree custody, and `git worktree prune`.
