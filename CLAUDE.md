...
- Clean up before releasing. The orphan hygiene check **fails both `--dry-run` and the real release** if a branch already merged into `main` still exists or an orphan worktree remains; `--skip-hygiene` is the only bypass. Delete merged branches, `aid accept` then `aid gc --task <id>` any task still holding worktree custody, and `git worktree prune`.
