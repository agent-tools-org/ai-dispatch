...
Built-in run agents: `gemini`, `agy`, `qwen`, `codex`, `copilot`, `opencode`, `cursor`, `kilo`, `mimocode`, `codebuff`, `droid`, `oz`, `claude`, and `grok`. Use `aid advise` to compare agents; there is no `auto` agent.
