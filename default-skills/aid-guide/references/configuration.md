...
A hold ends in one of four ways, and `aid config agents` names which:
...
pay. The last class is the opposite guard: an unrecognised refusal must not
take a route out permanently, so it expires by itself. An on-disk
