...
For remote Cargo work, use `aid run codex "Implement the change" --remote-build --worktree feat/change --dir . --bg`. Rbox sync includes untracked files and uses the same sanitized repo/branch checkout and remote target as `scripts/remote-test.sh`; the local `CARGO_TARGET_DIR` is never forwarded. Shim defaults are `AID_BUILD_JOBS` or 4 jobs, 3600 seconds timeout and 900 seconds lock timeout. A stderr heartbeat every 60 seconds keeps silent builds visible to the idle watcher. Exit 75 means the box lock was not acquired; exit 124 means the job is still running on the box.
