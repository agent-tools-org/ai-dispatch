...
fn parse_line(task_id: &TaskId, value: &Value, now: chrono::DateTime<Local>) -> Option<TaskEvent> {
...
fn usage_event(task_id: &TaskId, usage: &Value, now: chrono::DateTime<Local>) -> Option<TaskEvent> {
...
fn usage_detail(usage: &Value) -> Option<String> {
...
fn usage_metadata(usage: &Value) -> Option<Value> {
    let input = usage.get("inputTokens").and_then(Value::as_i64)?;
    let output = usage.get("outputTokens").and_then(Value::as_i64)?;
    let cached = usage
        .get("cacheReadTokens")
...
fn commandcode_result_failed(value: &Value) -> bool {
