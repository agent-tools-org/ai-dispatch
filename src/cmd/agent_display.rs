...
fn quota_row(kind: AgentKind) -> QuotaRow {
    ...
    let groups = rate_limit::active_group_holds(&kind);
    if groups.is_empty() {
        return QuotaRow::Ok;
    }
    QuotaRow::Partial {
...
pub(super) fn list_agents() -> anyhow::Result<()> {
    ...
    let rows: Vec<_> = AgentKind::ALL_BUILTIN
        .iter()
        .filter_map(|kind| kind.profile().map(|_| (*kind, quota_row(*kind))))
        .collect();
