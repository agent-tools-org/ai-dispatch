    ...
    F: FnOnce(),
{
    ...
    after_capture();
    let untracked = backup_untracked(repo_dir)?;
    if let Err(error) = clear_worktree(repo_dir) {
        if let Some(backup) = &untracked {
            let _ = restore_untracked(repo_dir, backup);
        }
        return Err(format_capture_error(tracked_ref.as_deref(), &error));
    }
    Ok(Some(LocalChanges { tracked_ref, untracked }))
}
...
pub(crate) fn format_stash_restore_error(changes: &LocalChanges, error: &str) -> String {
    let reference = changes
        .tracked_ref
        .clone()
        .or_else(|| changes.untracked.as_ref().map(|backup| backup.root.display().to_string()))
        .unwrap_or_else(|| "local-change backup".to_string());
    format!(
        "failed to restore merge-local changes ({reference}): {error}; recover manually before retrying"
    )
}
...
fn format_capture_error(reference: Option<&str>, error: &str) -> String {
    match reference {
        Some(reference) => format!("{error}; tracked changes remain recoverable as {reference}"),
