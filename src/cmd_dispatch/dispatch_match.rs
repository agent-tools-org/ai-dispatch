...
async fn dispatch_primary(store: Arc<crate::store::Store>, command: Commands) -> Result<()> {
    match command {
        Commands::Run(command_args_a::RunArgs { agent, prompt, repo, dir, output, model, budget, worktree, team, group, verify, judge, peer_review, retry, context, scope, run_extras, no_skill, bg, dry_run, read_only, sandbox, container, best_of, metric, parent, id, timeout, idle_timeout }) => handlers_a::run(store, agent, prompt, repo, dir, output, model, budget, worktree, team, group, verify, judge, peer_review, retry, context, scope, run_extras, no_skill, bg, dry_run, read_only, sandbox, container, best_of, metric, parent, id, timeout, idle_timeout).await,
        ...
        Commands::Show(command_args_a::ShowArgs { task_id, events: _, context, diff, summary, file, output, full, explain, log, json, agent, model }) => handlers_c::show(store, task_id, context, diff, summary, file, output, full, explain, log, json, agent, model).await,
...
async fn dispatch_secondary(store: Arc<crate::store::Store>, command: Commands) -> Result<()> {
    match command {
        ...
        Commands::Merge(command_args_b::MergeArgs { task_id, group, approve, check, target }) => {
            handlers_b::merge(store, task_id, group, approve, check, target)
        }
