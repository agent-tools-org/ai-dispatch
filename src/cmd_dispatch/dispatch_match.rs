...
async fn dispatch_primary(store: Arc<crate::store::Store>, command: Commands) -> Result<()> {
    match command {
        ...
        Commands::Batch(command_args_a::BatchArgs { action, file, vars, parallel, analyze, wait, dry_run, max_concurrent, output }) => handlers_a::batch(store, action, file, vars, parallel, analyze, wait, dry_run, max_concurrent, output).await,
...
async fn dispatch_secondary(store: Arc<crate::store::Store>, command: Commands) -> Result<()> {
    match command {
        ...
        Commands::Merge(command_args_b::MergeArgs { task_id, group, approve, check }) => handlers_b::merge(store, task_id, group, approve, check),
