...
async fn dispatch_primary(store: Arc<crate::store::Store>, command: Commands) -> Result<()> {
    match command {
        ...
        Commands::Show(command_args_a::ShowArgs { task_id, events, context, diff, summary, file, output, result, transcript, full, brief, explain, log, json, agent, model }) => handlers_c::show(store, task_id, events, context, diff, summary, file, output, result, transcript, full, brief, explain, log, json, agent, model).await,
...
async fn dispatch_secondary(store: Arc<crate::store::Store>, command: Commands) -> Result<()> {
    match command {
        Commands::Retry(command_args_b::RetryArgs { task_id, feedback, agent, dir, reset, bg }) => handlers_b::retry(store, task_id, feedback, agent, dir, reset, bg).await,
        ...
        Commands::Byok(command_args_c::ByokArgs { action }) => handlers_b::byok(action),
