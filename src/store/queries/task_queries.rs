...
impl Store {
    pub fn get_task(&self, id: &str) -> Result<Option<Task>> {
        ...
        let mut stmt = conn.prepare(
            "SELECT id, agent, prompt, resolved_prompt, status, parent_task_id, workgroup_id,
             ...
             exit_code, category, pending_reason, audit_verdict, audit_report_path, delivery_assessment, final_head_sha, final_branch, observed_model
    ...
    pub fn latest_default_model(&self, agent: AgentKind) -> Result<Option<String>> {
        ...
        for task in tasks {
            ...
            if let Some(raw) = task.attributed_model() {
